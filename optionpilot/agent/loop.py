...
OPTIONS_SYSTEM_PROMPT = (
    ...
    "buy&hold, IV vs realized vol, option volume + put/call ratio). For MARKET SENTIMENT / 市場情緒 "
    "/ VIX / 恐慌 regime: call regime_backtest, which reads the current VIX regime AND tests "
    "whether entering puts only in a chosen VIX percentile band beats the unfiltered strategy and "
    "buy&hold (a lookahead-free sentiment filter). When the user asks to see a "
...
CRYPTO_SYSTEM_PROMPT = (
    ...
    "outside the grid, fee + funding drag, all benchmarked against buy&hold; and (c) read market "
    "sentiment — market_sentiment: because these perps have US-STOCK underlyings, the relevant "
    "fear gauge is the equity VIX, NOT crypto sentiment. Funding is the contract-side signal; VIX "
    "is the underlying-equity side. grid_backtest also runs a VIX-regime variant (only add "
    "inventory when VIX is calm) and reports whether that gate helped. You do NOT do stock "
